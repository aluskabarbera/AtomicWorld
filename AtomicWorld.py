...
while True:
    ...
    if option1 == "1":
        ...
        if option2 == "1":
            ...
            print("\nI show you the bytes that have coincided with some subelement:\n")
            
            for byte in bytes.split(':'):
                for subelement in subelements:
                    if subelement['Binary'] == byte:
                        # We change the byte for the colored byte.
                        bytes = bytes.replace(byte, subelement['Color_binary'])
                        break
            print(bytes)
            ...
            for byte_colored in range(len(bytes.split(':'))):
                for subelement in subelements:
                    if subelement['Color_binary'] in bytes.split(':')[byte_colored]:
